...
* [Mockito vs. EasyMock](https://code.google.com/p/mockito/wiki/MockitoVSEasyMock)? Mockito's become a big hit with its motivation of testing states, while Java mock was dominated by expect-run-verify libraries like EasyMock and jMock -- before EasyMock came with NiceMock. Mockito is still slightly easier to read and modify, while we could stick with EasyMock's NiceMock. Even with Mockito, we'd be careful not to fall into the "testing interaction rather than testing state" trap. See also: [modern mocking tools](http://martinfowler.com/articles/modernMockingTools.html#Conclusions)
